...
router.delete("/admin/delete_message", (req, res) => {
  Messages.findOneAndDelete(req.body, { new: true })
    .then((message) => {
      if (!message) return res.status(404).send("package is not");
      res.send(message);
    })
    .catch((e) => {
      res.send(e);
...
router.delete("/admin/delete_user", (req, res) => {
  Users.findOneAndDelete(req.body, { new: true })
    .then((user) => {
      if (!user) return res.status(404).send("package is not");
      res.send(user);
    })
    .catch((e) => {
      res.send(e);
...
router.patch("/admin/update_subcategary", async (req, res) => {
  try {
    const subcategary = await SubCategaries.findOneAndUpdate(
      { _id: req.body.id },
      req.body,
      { new: true }
    );
...
router.delete("/admin/delete_subcategary", async (req, res) => {
  try {
    const subcategary = await SubCategaries.findOneAndDelete(req.body, {
      new: true,
    });
